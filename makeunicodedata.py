...
#
# This script converts Unicode database files to unicodedata_db.h,
# unicodename_db.h, and unicodetype_db.h
#
# history:
...
# note: should match definitions in unicodectype.c
ALPHA_MASK = 0x01
DECIMAL_MASK = 0x02
DIGIT_MASK = 0x04
LOWER_MASK = 0x08
...
def makeunicodedata(unicode, trace):
    ...
    with open(FILE, "w") as fp:
        ...
        fprint()
        fprint('let UNIDATA_VERSION = "%s"' % UNIDATA_VERSION)
        fprint("""
        struct UnicodeDataBaseRecord{
            let category:UInt8  //char /* index into _PyUnicode_CategoryNames */
            let combining:UInt8 //char /* combining class value 0 - 255 */
            let bidirectional:UInt8 //char  /* index into _PyUnicode_BidirectionalNames */
            let mirrored:UInt8      //char /* true if mirrored in bidir mode */
            let east_asian_width:UInt8 //char       /* index into _PyUnicode_EastAsianWidth */
            let normalization_quick_check:UInt8 //char /* see is_normalized() */
        }
        struct change_record {
            /* sequence of fields should be the same as in merge_old_version */
            let bidir_changed:UInt8 //char
            let category_changed:UInt8 //char
            let decimal_changed:UInt8 //char
            let mirrored_changed:UInt8 //char
            let east_asian_width_changed:UInt8 //char
            let numeric_changed:Double
        }

        """)
        fprint("/* a list of unique database records */")
        fprint("let UnicodeDatabaseRecords = [")
        for item in table:
            fprint(
                "    UnicodeDataBaseRecord(category:%d, combining:%d, bidirectional:%d, mirrored:%d, east_asian_width:%d, normalization_quick_check:%d)," % item)
        fprint("]")
        fprint()

        fprint("/* Reindexing of NFC first characters. */")
        fprint("let TOTAL_FIRST = ",total_first)
        ...
        struct reindex {
            ...
            var index:Int16}
        """)
        fprint("let nfc_first = [")
        for start,end in comp_first_ranges:
            fprint("    reindex(start:%d,count:%d,index:%d)," % (start,end-start,comp_first[start]))
        fprint("    reindex(start:0,count:0,index:0)")
        fprint("]\n")
        fprint("let nfc_last = [")
        for start,end in comp_last_ranges:
            fprint("  reindex(start:%d,count:%d,index:%d)," %
        ...
        fprint("let _PyUnicode_CategoryNames = [")
        for name in CATEGORY_NAMES:
            fprint("    \"%s\"," % name)
        fprint("    NULL")
        fprint("]")

        fprint("let _PyUnicode_BidirectionalNames = [")
        for name in BIDIRECTIONAL_NAMES:
            fprint("    \"%s\"," % name)
        fprint("    NULL")
        fprint("]")

        fprint("let _PyUnicode_EastAsianWidthNames = [")
        for name in EASTASIANWIDTH_NAMES:
            fprint("    \"%s\"," % name)
        fprint("    NULL")
        fprint("]")

        fprint("let decomp_prefix = [")
        ...
        fprint("let DECOMP_SHIFT = ", shift)
        Array("decomp_index1", index1).dump(fp, trace)
        Array("decomp_index2", index2).dump(fp, trace)

        index, index2, shift = splitbins(comp_data, trace)
        fprint("/* NFC pairs */")
        fprint("let COMP_SHIFT = ", shift)
        ...
        for version, table, normalization in unicode.changed:
            ...
            index1, index2, shift = splitbins(index, trace)
            fprint("let change_records_%s = [" % cversion)
            for record in records:
                fprint("    change_record(bidir_changed: %s,category_changed:%s,decimal_changed:%s,mirrored_changed:%s,east_asian_width_changed:%s,numeric_changed:%s )," % record)
            fprint("]")
            Array("changes_%s_index" % cversion, index1).dump(fp, trace)
            Array("changes_%s_data" % cversion, index2).dump(fp, trace)
            fprint("func get_change_%s(n:Py_UCS4) -> change_record {" % cversion)
            fprint("    var index:Int")
            ...
            fprint("}\n")
            fprint("func normalization_%s(n:Py_UCS4) -> Py_UCS4" % cversion)
            fprint("{")
            fprint("    switch n {")
...
def makeunicodetype(unicode, trace):
    ...
    with open(FILE, "w") as fp:
        ...
        fprint("""typealias Py_UCS4 = UInt32
typealias Py_UCS2 = UInt16
typealias Py_UCS1 = UInt8

""")
        
        fprint("""struct UnicodeTypeRecord {
            let upper: Int
            let lower: Int
            let title: Int
            ...
            """)
        fprint("let UnicodeTypeRecords = [")
        for item in table:
            fprint(
                "UnicodeTypeRecord(upper:%d, lower:%d, title:%d, decimal:%d, digit:%d, flags:%d)," % item)
        fprint("]")
        fprint()

        fprint("/* extended case mappings */")
        fprint()
        fprint("let UnicodeExtendedCase:[Py_UCS4] = [")
        ...
        fprint(' */')
        fprint('func UnicodeToNumeric(ch:Py_UCS4) -> Double')
        fprint('{')
        fprint('    switch ch {')
        for value, codepoints in numeric_items:
            ...
            codepoints.sort()
            for codepoint,has_more in lookahead(codepoints):
                fprint('    case 0x%04X:' % (codepoint,))
                if has_more:
                    fprint('        fallthrough')
            fprint('        return %s' % (value,))
        fprint('    default:')
        fprint('    return -1.0')
        fprint('    }')
        fprint('}')
        ...
        fprint(" */")
        fprint('func Unicode_IsWhitespace(ch:Py_UCS4) -> Bool')
        fprint('{')
        fprint('    switch ch {')

        for codepoint,has_more in lookahead(sorted(spaces)):
            fprint('    case 0x%04X:' % (codepoint,))
            if has_more:
                fprint('        fallthrough')
        fprint('        return true')
        ...
        fprint(" */")
        fprint('func IsLinebreak(ch: Py_UCS4) -> Bool')
        fprint('{')
        fprint('    switch ch {')
        for codepoint,has_more in lookahead(sorted(linebreaks)):
            fprint('    case 0x%04X:' % (codepoint,))
            if has_more:
                fprint('        fallthrough')
        fprint('        return true')
...
def makeunicodename(unicode, trace):
    ...
    with open(FILE, "w") as fp:
        ...
        fprint("let phrasebook_shift = ", shift)
        fprint("let phrasebook_short = ", short)
        ...
        fprint()
        fprint('let aliases_start:UInt = %#x;' %
               NAME_ALIASES_START)
        fprint('let aliases_end:UInt = %#x;' %
               (NAME_ALIASES_START + len(unicode.aliases)))

        fprint('let name_aliases:[UInt] = [')
        ...
        fprint('let named_sequences_start:UInt = %#x;' %
               NAMED_SEQUENCES_START)
        fprint('let named_sequences_end:UInt = %#x;' %
               (NAMED_SEQUENCES_START + len(unicode.named_sequences)))

        fprint('let named_sequences[] = [')
        for name, sequence in unicode.named_sequences:
            seq_str = ', '.join('0x%04X' % cp for cp in sequence)
            fprint('    named_sequence(seqlen:%d, seq:[%s]),' % (
                len(sequence), seq_str))
        fprint(']')
